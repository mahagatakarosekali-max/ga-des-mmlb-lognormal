...
def check_buffer_feasibility(individual):
    ...
    for buffer_name, capacity in buffer_allocation.items():
        if capacity < 0 or capacity > maximum_buffer_capacity:
            violations.append(
                f"{buffer_name} memiliki kapasitas {capacity}, di luar batas 0 sampai {maximum_buffer_capacity}."
            )
...
def show_feasibility_result(individual, is_feasible, violations):
    print("=" * 70)
    print("PEMERIKSAAN KELAYAKAN INDIVIDU")
    print("=" * 70)
    ...
    print("\nStatus kelayakan:")
    if is_feasible:
        print("Individu layak.")
    else:
        print("Individu tidak layak.")
